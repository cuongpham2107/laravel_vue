...
const search = ref(props.fillters.search);
const perPage = ref(props.fillters.perPage);
...
function getCasts() {
    Inertia.get(
        ...
        {
            preserveS9itate: true,
            replace: true,
        });
}
function generateCast(){
    Inertia.post(route('admin.casts.store'),
        {
           castTMDBId: castTMDBId.value,
        },
        {
            onFinish: () => (castTMDBId.value = ""),//kiểm tra tồn tại
        }
    );
